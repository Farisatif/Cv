...
const COMPANY_THEMES: Record<string, { bg: string; text: string; border: string }> = {
  ...
  GitHub:     { bg: "hsl(263 80% 68% / 0.10)", text: "hsl(263 72% 72%)",   border: "hsl(263 80% 68% / 0.20)" },
};
          ...
          {experience.map((exp, i) => {
            ...
            return (
              <div key={i} className={`relative ${isRTL ? "sm:pr-14" : "sm:pl-14"}`}>
                ...
                >
                  {isOpen ? (
                    <div className="w-[14px] h-[14px] rounded-full border-2 border-foreground bg-foreground timeline-dot-active dark:border-[hsl(263_80%_68%)] dark:bg-[hsl(263_80%_68%)] shadow-sm" />
                  ) : (
                    ...
                    <div className={`px-5 py-5 flex items-start gap-4 ${isRTL ? "flex-row-reverse" : ""}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className={`flex items-center gap-2 flex-wrap mb-1.5 ${isRTL ? "flex-row-reverse" : ""}`}>
                          ...
                          <span className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full bg-muted dark:bg-[hsl(263_80%_68%/0.08)] text-muted-foreground dark:text-[hsl(263_80%_72%)] font-mono border border-transparent dark:border-[hsl(263_80%_68%/0.12)]">
                            {exp.period}
                          </span>
                          ...
                          {exp.highlights.map((h, hi) => (
                            <li key={hi} className={`flex items-start gap-3 text-sm ${isRTL ? "flex-row-reverse" : ""}`}>
                              <span className={`mt-[2px] w-5 h-5 rounded-md flex items-center justify-center flex-shrink-0 ${
                                hi === 0
                                  ? "bg-foreground/10 dark:bg-[hsl(263_80%_68%/0.14)]"
                                  : "bg-foreground/6 dark:bg-[hsl(263_80%_68%/0.07)]"
                              }`}>
                                <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"
                                  className="dark:text-[hsl(263_80%_72%)] text-foreground/65">
