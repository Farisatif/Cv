      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {filtered.map((project, idx) => (
          <a
            ...
            className="group flex flex-col cosmic-card rounded-2xl p-5"
          ...
          >
            <div className={`flex items-start justify-between mb-3 gap-2 ${isRTL ? "flex-row-reverse" : ""}`}>
              ...
              <div className="text-muted-foreground opacity-0 group-hover:opacity-100 transition-all duration-200 flex-shrink-0 group-hover:-translate-y-0.5 group-hover:translate-x-0.5">
            ...
            <div className={`flex flex-wrap gap-1.5 mb-4 ${isRTL ? "flex-row-reverse" : ""}`}>
              {project.tags.map((tag) => (
                <span
                  key={tag}
                  className="px-2 py-0.5 rounded-full text-[10px] font-medium border border-border text-muted-foreground bg-muted/30 dark:bg-[hsl(263_80%_68%/0.06)] dark:border-[hsl(263_80%_68%/0.15)]"
                >
                  {tag}
                </span>
              ))}
