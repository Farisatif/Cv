...
export default function ContactSection() {
  ...
  return (
    ...
    >
      ...
      <div className={`grid grid-cols-1 lg:grid-cols-5 gap-5 ${isRTL ? "lg:flex-row-reverse" : ""}`}>
        <div className={`lg:col-span-2 ${isRTL ? "lg:order-2" : "lg:order-1"}`}>
          <div className="border border-border rounded-2xl bg-card p-6 h-full flex flex-col justify-between">
            <div>
              ...
              <p className={`text-sm text-muted-foreground leading-relaxed ${isRTL ? "text-right" : ""}`}>
                {lang === "ar"
                  ? "أفضّل مناقشة المشاريع عبر البريد الإلكتروني أو واتساب. أستجيب عادةً خلال 24 ساعة."
        ...
        <div className={`lg:col-span-3 ${isRTL ? "lg:order-1" : "lg:order-2"}`}>
          <div className="border border-border rounded-2xl bg-card overflow-hidden divide-y divide-border">
            {contactLinks.map((link) => (
              <div
                key={link.label}
                className={`px-5 py-4 flex items-center gap-4 hover:bg-muted/30 transition-colors group ${isRTL ? "flex-row-reverse" : ""}`}
              >
                <div className="w-9 h-9 rounded-xl border border-border bg-muted/50 flex items-center justify-center flex-shrink-0 text-muted-foreground group-hover:text-foreground group-hover:border-foreground/20 transition-all">
                  {link.icon}
                </div>
                ...
                <div className={`flex items-center gap-2 flex-shrink-0 ${isRTL ? "flex-row-reverse" : ""}`}>
                  ...
                  <a
                    ...
                    className="px-3 py-1.5 rounded-lg text-xs font-medium border border-border text-muted-foreground hover:border-foreground/30 hover:text-foreground transition-all duration-200 flex items-center gap-1.5 active:scale-95 whitespace-nowrap"
                  >
                    {link.actionLabel}
